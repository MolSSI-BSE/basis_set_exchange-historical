...
start_re = re.compile(r'^\s+?(\d+)[\s]+TOTAL NUMBER OF ATOM TYPES$')
# Element entry has two integers and text
element_re = re.compile(r'^\s+?(\d+)\s+(\d+)\s+ELECTRONIC AND NUCLEAR CHARGE$')
# Number of primitives
nprim_re = re.compile(r'^\s+?(\d+)\s+NUMBER OF BARE GAUSSIANS$')
# Number of s, p, and d functions
nspd_re = re.compile(r'^\s+?(\d+)\s+(\d+)\s+(\d+)\s+NUMBER OF S,P,D FUNCTIONS$')
# Number of supplementary functions
nsup_re = re.compile(r'^\s+?(\d+)\s+(\d+)\s+(\d+)\s+SUPPLEMENTARY S,P,D FUNCTIONS$')
...
def read_nrlmol(basis_lines, extra=False):
    ...
    iline=0
    while not start_re.match(basis_lines[iline]):
        iline += 1
    ...
    print('{} element sections {} types'.format(len(element_sections), ntypes))
